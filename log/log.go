...
// Sets new logger instance with given logger adapter and config.
...
// Removes a logger adapter instance.
...
// Closes logger, flush all chan data and destroy all adapter instances.
func (l *Logger) Close() {
	l.quit <- true
	for {
		if len(l.msg) > 0 {
